/**
 * Master UI Prompt - KW APPS Styling Guidelines
 *
 * This prompt ensures AI-generated code matches brand identity
 * and avoids generic "AI slop" aesthetic.
 */

export const MASTER_UI_PROMPT = `
# KW APPS Master UI Generation Guidelines

You are generating React code for KW APPS, an Arabic-first website builder platform.
Your generated code MUST follow these strict design and technical requirements.
...
Your goal is to generate **production-ready, Arabic-first, brand-consistent** React code
that feels intentionally designed by a human, not automatically generated by AI.

Be creative within the brand constraints. Surprise and delight with thoughtful layouts,
purposeful animations, and professional aesthetics.
`

// Helper function to inject the prompt into DeepSeek requests
export function getMasterUIPromptWithContext(userPrompt: string): string {
...
Generate React code that fulfills the user's request while strictly following ALL guidelines above.
`
}

// Export as const for re-use
export const RTL_VERIFICATION_PROMPT = `
Review the following React code and ensure it meets KW APPS RTL and Arabic requirements:

1. Root element has \`dir="rtl"\`
2. All text content is in Arabic (no English except technical labels)
3. All text elements have \`text-right\`
4. RTL-aware spacing (mr not ml, pr not pl)
5. Cairo font is loaded and applied
6. Navigation flows right-to-left
7. Icons are appropriately oriented for RTL

If any issues are found, fix them and return the corrected code.
If the code is already compliant, return it unchanged.
`

export const SECURITY_VALIDATION_PROMPT = `
Review the following React code for security vulnerabilities:

1. No \`eval()\` or \`Function()\` calls
2. No \`dangerouslySetInnerHTML\` with user input
3. No external API calls (fetch, axios, etc.)
4. No inline event handlers with string code
5. No XSS vulnerabilities
6. No script injection possibilities
7. Only safe React patterns

If any security issues are found, fix them and return the corrected code.
If the code is secure, return it unchanged.
`
